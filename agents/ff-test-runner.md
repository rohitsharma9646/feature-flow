...
5. If a regression test path is provided (bugfix track), run it specifically and report RED/GREEN with the exit code for each.
   If an end-to-end check is provided (feature track), run exactly that command / flow as stated — on a lite-tier dispatch too, the one carve-out from lite's floor-only rule — and record it under its true kind (`cli-output`, `http/api`, `e2e/browser`, …), bounded like any other command.

...
| single-architect-guard.sh | 0 (full output saved separately: cli-output-2-single-architect-guard.md) |
...
## integrity-conformance-guard.sh output

```
$ bash scripts/checks/integrity-conformance-guard.sh
FAIL: Go toolchain is required for WP1 conformance
EXIT_CODE=1
```

## Full log of all 27 guards (concatenated stdout+stderr per guard)

See raw capture below (also mirrors /tmp/guards_all.log + /tmp/integrity_conformance.log from this
run). Only the header/exit lines are excerpted here for brevity since the guards are verbose
"ok:"-style checklists; full "ok:" lines are in the raw capture appended below.
...
ok:   commands/ff-design.md: adversarial beat (116) sits between the do-not-contradict STOP (114) and '## Write the artifact' (140)
...
ok:   rev lib/FS4: durable form {"paths":{"durable":"/tmp/tmp.Y1NnmvTbBK/rev-repo/docs/ff"}} is excluded
...
EXIT_CODE=0
=== integrity-conformance-guard.sh (skipped in main loop, requires Go; run separately) ===

...
result. A manifest rewritten through a shell command (`Bash`) is **not** covered: shell text
...
· an Edit whose target file is missing or whose `old_string` is not found — Claude Code rejects
that edit itself · `toggles.enforce: false`). It is **Claude-Code-only** — the Codex package excludes `hooks/`, so

...
- **Feature:** the done-transition is always `ff-verify` (verify is feature-terminal).
- **Bugfix:** verify and review can run in either order; the done-transition is whichever runs
  **second** — normally `ff-review`, but `ff-verify`'s convergence branch when review ran first.
  `ff-review` carries a **feature-track guard** (on the feature track review precedes verify, so
  `ff-review` skips capture — it happens at `ff-verify`). Firing only at the done-transition
  guarantees capture happens exactly once and never double-captures.
